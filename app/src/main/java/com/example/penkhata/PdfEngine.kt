...
// --- NEW: INDIAN NUMBER TO WORDS ALGORITHM ---
...
fun createPdf(ctx: Context, isQuote: Boolean, invNo: String, date: String, payMode: String, delNote: String,
    ...
    // FIX 1: SMART GSTIN DISPLAY
    var buyerInfo = ""
    if (bGst.isNotEmpty()) buyerInfo += "GSTIN: $bGst  "
    if (bState.isNotEmpty()) buyerInfo += "State: $bState"
    if (buyerInfo.isNotEmpty()) c.drawText(buyerInfo, m+5, r1+rH/2+60, p)
    ...
    items.forEachIndexed { i, it ->
        // Row Height Calculation (Dynamic)
        val rowHeight = if(it.serial.isNotEmpty()) 30f else 20f

        val rowInc = it.qty * it.rate
        val taxF = 1 + (it.taxRate/100)
        val rowBase = rowInc/taxF
        ...
        // FIX 2: SERIAL NUMBER PADDING
        if(it.serial.isNotEmpty()) {
            val ps=Paint(p); ps.textSize=8f
            c.drawText("Sr/IMEI: ${it.serial}", c2+5, y+26, ps) // Shifted down for cleaner look
        }

        p.textAlign=Paint.Align.CENTER
        c.drawText(it.hsn, c3+w3/2, y+14, p)
    ...
    c.drawText("Grand Total", tX-10, y+14, p); c.drawText("₹ ${String.format("%.0f", gTotal)}", m+w-5, y+14, p); c.drawLine(tX, y, tX, m+h, bp)

    // Footer
    val fY = fTop+20; p.textAlign=Paint.Align.LEFT; p.isFakeBoldText=false

    // FIX 3: AMOUNT IN WORDS
    c.drawText("Amount in words: ${convertToWords(gTotal.toLong())}", m+5, fY, p)
    ...
    // FIX 4: SIGNATORY ALIGNMENT
    val sigY = decY; c.drawLine(c5, sigY, tX, sigY, bp)
    p.textSize=10f; p.isFakeBoldText=true; p.textAlign=Paint.Align.CENTER
    val signBoxStart = c5
    val signBoxEnd = m + w // Right edge of page border
    val signCenterX = (signBoxStart + signBoxEnd) / 2 // Perfect Center
